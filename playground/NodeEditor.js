...
export class NodeEditor extends THREE.EventDispatcher {
	...
	_initExamplesContext() {
		...
		addExamples("Universal", ["Teapot", "Matcap", "Fresnel"]);

		if (this.renderer.isWebGLRenderer) {
			addExamples("WebGL", ["Car"]);

			context.add(
				new ButtonInput("WebGPU Version").onClick(() => {
					if (confirm("Are you sure?") === true) {
			...
			addExamples("WebGPU", ["Particle"]);

			context.add(
				new ButtonInput("WebGL Version").onClick(() => {
					if (confirm("Are you sure?") === true) {
