...
export const BurnNode = tslFn(({ base, blend }) => {
	const fn = (c) =>
		blend[c]
			.lessThan(EPSILON)
			.cond(blend[c], base[c].oneMinus().div(blend[c]).oneMinus().max(0));
...
export const DodgeNode = tslFn(({ base, blend }) => {
	const fn = (c) =>
		blend[c].equal(1.0).cond(blend[c], base[c].div(blend[c].oneMinus()).max(0));
	...
	const fn = (c) =>
		base[c]
			...
			.cond(
				...
				base[c].oneMinus().mul(blend[c].oneMinus()).oneMinus()
			);
