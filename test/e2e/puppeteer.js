...
async function main() {
	...
	try {
		await fs.rm("test/e2e/output-screenshots", {
			recursive: true,
			force: true,
		});
	} catch {}
	...
	const exactList = process.argv
		.slice(isMakeScreenshot ? 3 : 2)
	...
	let files = (await fs.readdir("examples"))
		...
		.filter((f) =>
			isExactList ? exactList.includes(f) : !exceptionList.includes(f)
		);
	...
	const build = (await fs.readFile("build/three.module.js", "utf8")).replace(
	...
	if (isMakeScreenshot && failedScreenshots.length) {
		...
		console.red(
			`If you are sure that everything is correct, try to run "npm run make-screenshot ${list}". If this does not help, try increasing idleTime and parseTime variables in /test/e2e/puppeteer.js file. If this also does not help, add remaining screenshots to the exception list.`
		);
		console.red(
			`${failedScreenshots.length} from ${files.length} screenshots have not generated succesfully.`
		);
		...
		console.red(
			`TEST FAILED! ${failedScreenshots.length} from ${files.length} screenshots have not rendered correctly.`
		);
	} else {
		console.green(
			`TEST PASSED! ${files.length} screenshots rendered correctly.`
		);
	}
...
async function makeAttempt(
	...
	try {
		...
		if (isMakeScreenshot) {
			...
			try {
				expected = (
					await jimp.read(`examples/screenshots/${file}.jpg`)
				).quality(jpgQuality);
			} catch {
				await screenshot.writeAsync(
					`test/e2e/output-screenshots/${file}-actual.jpg`
				);
			...
			try {
				...
				await expected.writeAsync(
					`test/e2e/output-screenshots/${file}-expected.jpg`
				);
			...
			if (differentPixels < maxDifferentPixels) {
				...
				throw new Error(
					`Diff wrong in ${differentPixels.toFixed(
						1
