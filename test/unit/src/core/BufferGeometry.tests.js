...
export default QUnit.module("Core", () => {
	QUnit.module("BufferGeometry", () => {
		...
		QUnit.test("applyQuaternion", (assert) => {
			...
			assert.ok(
				pos[0] === 3 &&
					...
					pos[4] === 4 &&
					pos[5] === 5,
				"vertices were rotated properly"
			);
		});
		...
		QUnit.test("center", (assert) => {
			...
			geometry.setAttribute(
				"position",
				new BufferAttribute(new Float32Array([-1, -1, -1, 1, 1, 1, 4, 4, 4]), 3)
			);
		...
		QUnit.test("computeBoundingBox", (assert) => {
			let bb = getBBForVertices([
				-1, -2, -3, 13, -2, -3.5, -1, -20, 0, -4, 5, 6,
			]);
		...
		QUnit.test("computeBoundingBox - Float16", (assert) => {
			...
			geometry.setAttribute(
				"position",
				new Float16BufferAttribute(toHalfFloatArray(vertices), 3)
			);
			geometry.computeBoundingBox();
		...
		QUnit.test("computeVertexNormals", (assert) => {
			...
			const difference = direction.dot(
