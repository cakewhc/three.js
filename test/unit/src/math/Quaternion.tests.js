...
function slerpTestSkeleton(doSlerp, maxError, assert) {
	...
	const a = [
		0.6753410084407496, 0.4087830051091744, 0.32856700410659473,
		0.5185120064806223,
	];

	const b = [
		0.6602792107657797, 0.43647413932562285, 0.35119011210236006,
		0.5001871596632682,
	];
...
export default QUnit.module("Maths", () => {
	QUnit.module("Quaternion", () => {
		...
		QUnit.test("setFromAxisAngle", (assert) => {
			...
			const b1 = new Quaternion().setFromAxisAngle(
				new Vector3(1, 0, 0),
				Math.PI
			);
			assert.ok(!a.equals(b1), "Passed!");
			const b2 = new Quaternion().setFromAxisAngle(
				new Vector3(1, 0, 0),
				-Math.PI
			);
		...
		QUnit.test("multiplyQuaternions/multiply", (assert) => {
			...
			const q3 = new Quaternion().setFromEuler(
				changeEulerOrder(angles[2], "XYZ")
			);

			const q = new Quaternion().multiplyQuaternions(q1, q2).multiply(q3);

			const m1 = new Matrix4().makeRotationFromEuler(
				changeEulerOrder(angles[0], "XYZ")
			);
			const m2 = new Matrix4().makeRotationFromEuler(
				changeEulerOrder(angles[1], "XYZ")
			);
			const m3 = new Matrix4().makeRotationFromEuler(
				changeEulerOrder(angles[2], "XYZ")
			);
