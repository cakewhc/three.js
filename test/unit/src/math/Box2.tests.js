...
export default QUnit.module("Maths", () => {
	QUnit.module("Box2", () => {
		...
		QUnit.test("intersect", (assert) => {
			...
			assert.ok(
				e.min.equals(posInf2) && e.max.equals(negInf2),
				"Infinite empty"
			);
		});
		...
		QUnit.test("translate", (assert) => {
			...
			assert.ok(
				a.clone().translate(one2).equals(new Box2(one2, one2)),
				"Passed!"
			);
			assert.ok(
				a.clone().translate(one2).translate(one2.clone().negate()).equals(a),
				"Passed!"
			);
			assert.ok(c.clone().translate(one2).equals(b), "Passed!");
			assert.ok(
				b.clone().translate(one2.clone().negate()).equals(c),
				"Passed!"
			);
		});
