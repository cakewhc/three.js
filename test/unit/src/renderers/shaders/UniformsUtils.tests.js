	...
	QUnit.module("Shaders", () => {
		QUnit.module("UniformsUtils", () => {
			...
			QUnit.test("cloneUniforms copies values", (assert) => {
				...
				assert.ok(
					uniforms.colorValue.value.equals(uniformClones.colorValue.value)
				);
				assert.ok(
					uniforms.vector2Value.value.equals(uniformClones.vector2Value.value)
				);
				assert.ok(
					uniforms.vector3Value.value.equals(uniformClones.vector3Value.value)
				);
				assert.ok(
					uniforms.vector4Value.value.equals(uniformClones.vector4Value.value)
				);
				assert.ok(
					uniforms.matrix3Value.value.equals(uniformClones.matrix3Value.value)
				);
				assert.ok(
					uniforms.matrix4Value.value.equals(uniformClones.matrix4Value.value)
				);
				assert.ok(
					uniforms.quatValue.value.equals(uniformClones.quatValue.value)
				);
				assert.ok(
					uniforms.textureValue.value.source.uuid ===
						uniformClones.textureValue.value.source.uuid
				);
				assert.ok(
					uniforms.textureValue.value.mapping ===
						uniformClones.textureValue.value.mapping
				);
				for (let i = 0; i < uniforms.arrayValue.value.length; ++i) {
					assert.ok(
						uniforms.arrayValue.value[i] === uniformClones.arrayValue.value[i]
					);
				}
			});
			...
			QUnit.test("cloneUniforms clones properties", (assert) => {
				...
				assert.ok(
					uniforms.arrayValue.value[0] !== uniformClones.arrayValue.value[0]
				);
