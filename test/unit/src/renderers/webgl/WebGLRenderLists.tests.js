...
import {
	WebGLRenderLists,
	WebGLRenderList,
} from "../../../../../src/renderers/webgl/WebGLRenderLists.js";
		...
		QUnit.module("WebGLRenderLists", () => {
			...
			QUnit.test("get", (assert) => {
				...
				assert.propEqual(
					listA,
					new WebGLRenderList(),
					"listA is type of WebGLRenderList."
				);
				assert.propEqual(
					listB,
					new WebGLRenderList(),
					"listB is type of WebGLRenderList."
				);
		...
		QUnit.module("WebGLRenderList", () => {
			QUnit.test("init", (assert) => {
				...
				assert.ok(
					...
					"Transparent list defaults to length 0."
				);
				assert.ok(
					list.opaque.length === 0,
					"Opaque list defaults to length 0."
				);
				...
				assert.ok(
					list.transparent.length === 1,
					"Transparent list is length 1 after adding transparent item."
				);
				assert.ok(
					list.opaque.length === 1,
					"Opaque list list is length 1 after adding opaque item."
				);
				...
				assert.ok(
					list.transparent.length === 0,
					"Transparent list is length 0 after calling init."
				);
				assert.ok(
					list.opaque.length === 0,
					"Opaque list list is length 0 after calling init."
				);
			});
			...
			QUnit.test("push", (assert) => {
				...
				assert.ok(
					list.opaque.length === 0,
					"Opaque list list is length 0 after adding transparent item."
				);
				assert.deepEqual(
					list.transparent[0],
					{
				...
				assert.ok(
					list.transparent.length === 2,
					"Transparent list is length 2 after adding second transparent item."
				);
				assert.ok(
					list.opaque.length === 0,
					"Opaque list list is length 0 after adding second transparent item."
				);
				assert.deepEqual(
					list.transparent[1],
					{
				...
				assert.ok(
					list.transparent.length === 2,
					"Transparent list is length 2 after adding first opaque item."
				);
				assert.ok(
					list.opaque.length === 1,
					"Opaque list list is length 1 after adding first opaque item."
				);
				assert.deepEqual(
					list.opaque[0],
					{
				...
				assert.ok(
					list.transparent.length === 2,
					"Transparent list is length 2 after adding second opaque item."
				);
				assert.ok(
					list.opaque.length === 2,
					"Opaque list list is length 2 after adding second opaque item."
				);
				assert.deepEqual(
					list.opaque[1],
					{
